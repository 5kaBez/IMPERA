...
// ===== Геолокация (Haversine formula) =====

const GEO_RADIUS_M = 1000; // 1 км — радиус для гео-проверки
...
/**
 * Проверяет, находится ли студент в пределах GEO_RADIUS от зала.
 * Возвращает { dist, geoOk }.
 */
export function checkGeo(
  studentLat: number | null, studentLon: number | null,
  gymLat: number | null, gymLon: number | null
): { dist: number; geoOk: boolean } {
  if (studentLat == null || studentLon == null || gymLat == null || gymLon == null) {
    return { dist: -1, geoOk: false };
  }
  const dist = Math.round(geoDist(studentLat, studentLon, gymLat, gymLon));
  return { dist, geoOk: dist <= GEO_RADIUS_M };
}
